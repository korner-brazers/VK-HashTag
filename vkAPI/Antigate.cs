...
namespace vkAPI
{
    ...
    {
        public static Task<double> GetBalance 
        {
            get 
            {
                return Task.Run(() =>
                    {
                        try
                        {
                            //Обычный балас антикапчи
                            return Math.Round(double.Parse(((GetBrowser)Browser.Get("http://antigate.com/res.php?key=" + VKdata.KeyAntigate + "&action=getbalance")).html.Trim().Replace(".", ",")), 2);
                        }
                        catch { return 0; }
                    });
            }
        ...
        public string get(string url)
        {
            ...
            string id = UploadCAPCHA(url), key = "";
            if (id == null || id.Trim() == "")
                return "";
            ...
            //Возвращаем данные в формате контакта
            return string.Format("&captcha_sid={0}&captcha_key={1}", new Regex("sid=([0-9]+)").Match(url).Groups[1].Value.Trim(), key.Trim()).Trim();
        }
        ...
        private string HttpGet(string CaptchaId)
        {
            ...
            try
            {
                Request = (HttpWebRequest)WebRequest.Create("http://antigate.com/res.php?key=" + VKdata.KeyAntigate + "&action=get&id=" + CaptchaId);
